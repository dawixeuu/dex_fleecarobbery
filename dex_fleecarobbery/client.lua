...
Freeze = {F0 = 0, F1 = 0, F2 = 0, F3 = 0, F4 = 0, F5 = 0, F6 = 0}
PlayerData = nil
Check = {F0 = false, F1 = false, F2 = false, F3 = false, F4 = false, F5 = false, F6 = false}
SearchChecks = {F0 = false, F1 = false, F2 = false, F3 = false, F4 = false, F5 = false, F6 = false}
LootCheck = {
    F0 = {Stop = false, Loot1 = false, Loot2 = false, Loot3 = false},
...
function ShowTimer() SetTextFont(0) SetTextProportional(0) SetTextScale(0.42, 0.42) SetTextDropShadow(0, 0, 0, 0,255) SetTextEdge(1, 0, 0, 0, 255) SetTextEntry("STRING") AddTextComponentString("~r~"..DEX.timer.."~w~") DrawText(0.682, 0.96) end

--[[RegisterNetEvent("DEX_fh:openDoor_c")
AddEventHandler("DEX_fh:openDoor_c", function(coords, method)
    if method == 1 then
        local obj = GetClosesDEXjectOfType(coords, 2.0, GetHashKey(DEX.vaultdoor), false, false, false)
        local count = 0
    ...
    elseif method == 2 then
        local obj = GetClosesDEXjectOfType(DEX.Banks.F4.doors.startloc.x, DEX.Banks.F4.doors.startloc.y, DEX.Banks.F4.doors.startloc.z, 2.0, DEX.Banks.F4.hash, false, false, false)
        local count = 0
        repeat
            local heading = GetEntityHeading(obj) - 0.10
    ...
    elseif method == 3 then
        local obj = GetClosesDEXjectOfType(coords, 2.0, GetHashKey(DEX.door), false, false, false)
...
RegisterNetEvent("DEX_fh:resetDoorState")
AddEventHandler("DEX_fh:resetDoorState", function(name)
    Freeze[name] = 0
end)

RegisterNetEvent("DEX_fh:lootup_c")
AddEventHandler("DEX_fh:lootup_c", function(var, var2)
    LootCheck[var][var2] = true
end)

RegisterNetEvent("DEX_fh:outcome")
AddEventHandler("DEX_fh:outcome", function(oc, arg)
    for i = 1, #Check, 1 do
        Check[i] = false
    end
    ...
    if oc then
        Check[arg] = true
        TriggerEvent("DEX_fh:startheist", DEX.Banks[arg], arg)
    elseif not oc then
        exports["mythic_notify"]:SendAlert("error", arg)
    end
end)

RegisterNetEvent("DEX_fh:startLoot_c")
AddEventHandler("DEX_fh:startLoot_c", function(data, name)
    --local check = true
    --[[while check do
                ...
                if dst < 40 then
                    if not LootCheck[name].Loot1 then
                        ...
                        if dst1 < 5 then
                            ...
                            if dst1 < 0.75 and IsControlJustReleased(0, 38) then
                                TriggerServerEvent("DEX_fh:lootup", name, "Loot1")
                                StartGrab(name)
                            end
                        end
                    end
                    ...
                    if not LootCheck[name].Loot2 then
                        ...
                        if dst1 < 5 then
                            ...
                            if dst1 < 1 and IsControlJustReleased(0, 38) then
                                TriggerServerEvent("DEX_fh:lootup", name, "Loot2")
                                StartGrab(name)
                            end
                        end
                    end
                    ...
                    if not LootCheck[name].Loot3 then
                        ...
                        if dst1 < 5 then
                            ...
                            if dst1 < 1 and IsControlJustReleased(0, 38) then
                                TriggerServerEvent("DEX_fh:lootup", name, "Loot3")
                                StartGrab(name)
                            end
                        end
                    end
                    ...
                    if LootCheck[name].Stop or (LootCheck[name].Loot1 and LootCheck[name].Loot2 and LootCheck[name].Loot3) then
                        ...
                        if initiator then
                            TriggerEvent("DEX_fh:reset", name, data)
                            return
                        end
                        return
                    end
...
RegisterNetEvent("DEX_fh:stopHeist_c")
AddEventHandler("DEX_fh:stopHeist_c", function(name)
    LootCheck[name].Stop = true
end)

RegisterNetEvent("DEX_fh:policenotify")
AddEventHandler("DEX_fh:policenotify", function(name)
    ...
    if PlayerData.job.name == "police" then
        ...
        if not DoesBlipExist(blip) then
            blip = AddBlipForCoord(DEX.Banks[name].doors.startloc.x, DEX.Banks[name].doors.startloc.y, DEX.Banks[name].doors.startloc.z)
...
AddEventHandler("DEX_fh:freezeDoors", function()
    Citizen.CreateThread(function()
        while true do
            for k, v in pairs(Doors) do
                if v[1].obj == nil or not DoesEntityExist(v[1].obj) then
                    v[1].obj = GetClosesDEXjectOfType(v[1].loc, 1.5, GetHashKey("v_ilev_gb_vaubar"), false, false, false)
                    ...
                    for i = 1, 2, 1 do
                        ...
                        if dst <= 4.0 then
                            ...
                            if dst <= 1.5 and IsControlJustReleased(0, 38) then
                                dooruse = true
                                if i == 2 then
                                    TriggerServerEvent("DEX_fh:toggleVault", k, not v[i].locked)
                                else
                                    TriggerServerEvent("DEX_fh:toggleDoor", k, not v[i].locked)
                                end
                            end
                        end
                    end
                ...
                for k, v in pairs(Doors) do
                    if GetDistanceBetweenCoords(v[2].loc, pcoords, true) <= 20.0 then
                        if v[2].state ~= nil then
                            local obj
                            if k ~= "F4" then
                                obj = GetClosesDEXjectOfType(v[2].loc, 1.5, GetHashKey("v_ilev_gb_vauldr"), false, false, false)
                            else
                                obj = GetClosesDEXjectOfType(v[2].loc, 1.5, 4231427725, false, false, false)
                            end
...
RegisterNetEvent("DEX_fh:toggleDoor")
AddEventHandler("DEX_fh:toggleDoor", function(key, state)
    Doors[key][1].locked = state
    dooruse = false
end)

RegisterNetEvent("DEX_fh:toggleVault")
AddEventHandler("DEX_fh:toggleVault", function(key, state)
    dooruse = true
    Doors[key][2].state = nil
    if DEX.Banks[key].hash == nil then
        if not state then
            local obj = GetClosesDEXjectOfType(DEX.Banks[key].doors.startloc.x, DEX.Banks[key].doors.startloc.y, DEX.Banks[key].doors.startloc.z, 2.0, GetHashKey(DEX.vaultdoor), false, false, false)
            local count = 0
            ...
            TriggerServerEvent("DEX_fh:updateVaultState", key, Doors[key][2].state)
        elseif state then
            local obj = GetClosesDEXjectOfType(DEX.Banks[key].doors.startloc.x, DEX.Banks[key].doors.startloc.y, DEX.Banks[key].doors.startloc.z, 2.0, GetHashKey(DEX.vaultdoor), false, false, false)
            local count = 0
...
AddEventHandler("DEX_fh:reset", function(name, data)
    ...
    TriggerServerEvent("DEX_fh:toggleVault", name, true)
    TriggerEvent("DEX_fh:cleanUp", data, name)
end)

AddEventHandler("DEX_fh:startheist", function(data, name)
    TriggerServerEvent("DEX_fh:toggleDoor", name, true) -- ensure to lock the second door for the second, third, fourth... heist
    disableinput = true
    currentname = name
    currentcoords = vector3(data.doors.startloc.x, data.doors.startloc.y, data.doors.startloc.z)
    initiator = true
    ...
    disableinput = false
    Citizen.Wait(1000)
    Process(DEX.hacktime, "Hack in Progress")
    exports["mythic_notify"]:SendAlert("success", "Hacking complete!")
    PlaySoundFrontend(-1, "ATM_WINDOW", "HUD_FRONTEND_DEFAULT_SOUNDSET")
    TriggerServerEvent("DEX_fh:toggleVault", name, false)
    startdstcheck = true
    currentname = name
...
AddEventHandler("DEX_fh:cleanUp", function(data, name)
    Citizen.Wait(10000)
    for i = 1, 3, 1 do -- full trolley clean
        local obj = GetClosesDEXjectOfType(data.objects[i].x, data.objects[i].y, data.objects[i].z, 0.75, GetHashKey("hei_prop_hei_cash_trolly_01"), false, false, false)
    ...
    for j = 1, 3, 1 do -- empty trolley clean
        local obj = GetClosesDEXjectOfType(data.objects[j].x, data.objects[j].y, data.objects[j].z, 0.75, GetHashKey("hei_prop_hei_cash_trolly_03"), false, false, false)
    ...
    TriggerServerEvent("DEX_fh:setCooldown", name)
    initiator = false
end)
...
function SpawnTrolleys(data, name)
    ...
    SetEntityHeading(Trolley1, h1 + DEX.Banks[name].trolley1.h)
    SetEntityHeading(Trolley2, h2 + DEX.Banks[name].trolley2.h)
    SetEntityHeading(Trolley3, h3 + DEX.Banks[name].trolley3.h)
    ...
    TriggerServerEvent("DEX_fh:startLoot", data, name, missionplayers)
    done = false
end
...
function StartGrab(name)
    ...
    Trolley = GetClosesDEXjectOfType(GetEntityCoords(ped), 1.0, GetHashKey("hei_prop_hei_cash_trolly_01"), false, false, false)
		    ...
		    while GetGameTimer() - startedGrabbing < 37000 do
			    ...
			    if HasAnimEventFired(ped, GetHashKey("RELEASE_CASH_DESTROY")) then
				    if IsEntityVisible(grabobj) then
                        SetEntityVisible(grabobj, false, false)
                        TriggerServerEvent("DEX_fh:rewardCash")
				    end
			    end
		    end
    ...
    --TriggerServerEvent("DEX_fh:updateObj", name, NewTrolley, 2)
        ...
        if startdstcheck then
            if initiator then
                ...
                if (GetDistanceBetweenCoords(playercoord, currentcoords, true)) > 20 then
                    ...
                    startdstcheck = false
                    TriggerServerEvent("DEX_fh:stopHeist", currentname)
                end
            end
            Citizen.Wait(1)
        else
    ...
    while true do
        if startdstcheck then
            if initiator then
                if DEX.timer > 0 then
                    Citizen.Wait(1000)
                    DEX.timer = DEX.timer - 1
                elseif DEX.timer == 0 then
                    startdstcheck = false
                    TriggerServerEvent("DEX_fh:stopHeist", currentname)
                    DEX.timer = resettimer
                end
            end
            Citizen.Wait(1)
        else
...
Citizen.CreateThread(function()
    ...
    ESX.TriggerServerCallback("DEX_fh:getBanks", function(bank, door)
        DEX.Banks = bank
        Doors = door
    end)
    --Citizen.Wait(1000)
    TriggerEvent("DEX_fh:freezeDoors")
    while true do
        if PlayerData.job.name ~= "police" then
            local coords = GetEntityCoords(PlayerPedId())

            for k, v in pairs(DEX.Banks) do
                if not v.onaction then
                    ...
                    if dst <= 5 and not Check[k] then
                        ...
                        if dst <= 1 and IsControlJustReleased(0, 38) then
                            TriggerServerEvent("DEX_fh:startcheck", k)
                        end
                    end
